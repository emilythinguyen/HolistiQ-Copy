...
class FormCheckChatbox {
  ...
  init() {
    ...
    // Get the chatbox wrapper for external access
    this.chatboxWrapper = this.container.querySelector('.chatbox-wrapper');
  ...
  show() {
    if (this.chatboxWrapper) {
      this.chatboxWrapper.classList.remove('hidden');
    }
  }
  
  hide() {
    if (this.chatboxWrapper) {
      this.chatboxWrapper.classList.add('hidden');
    }
  }
  
  toggle() {
    if (this.chatboxWrapper) {
      this.chatboxWrapper.classList.toggle('hidden');
    }
  }
}
...
// Initialize the chatbox when document is loaded
document.addEventListener('DOMContentLoaded', function() {
  // Initialize the chatbox
  const chatboxContainer = document.getElementById('form-check-chatbox');
  if (chatboxContainer) {
    const chatbox = new FormCheckChatbox('#form-check-chatbox');
    
    // Make the chatbox toggleable
    ...
    // Store the chatbox instance globally for easier access
    window.formCheckChatbox = chatbox;
    
    // Add popup class to the wrapper
    if (chatbox.chatboxWrapper) {
      chatbox.chatboxWrapper.classList.add('popup', 'hidden');
    }
    
    // Toggle button event
    if (chatboxToggleBtn) {
      chatboxToggleBtn.addEventListener('click', function() {
        if (window.formCheckChatbox) {
          window.formCheckChatbox.toggle();
        }
      });
    }
    
    // Open chatbox when "Try Form Check AI" button is clicked
    if (openChatboxBtn) {
      openChatboxBtn.addEventListener('click', function(e) {
        e.preventDefault();
        if (window.formCheckChatbox) {
          window.formCheckChatbox.show();
        }
      });
    }
  }
